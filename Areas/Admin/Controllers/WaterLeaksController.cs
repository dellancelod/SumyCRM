...
namespace SumyCRM.Areas.Admin.Controllers
{
    ...
    public class WaterLeaksController : Controller
    {
        ...
        private static string NormalizeStreetName(string s)
        {
            ...
            // Remove common UA prefixes/suffixes that users type
            var remove = new[]
            {
                "вул.", "вулиця", "ул.", "улица", "просп.", "проспект",
                "площа", "пл.", "пров.", "провулок", "бульвар", "бул.", "набережна"
            };

            foreach (var r in remove)
                s = System.Text.RegularExpressions.Regex.Replace(s, $@"\b{System.Text.RegularExpressions.Regex.Escape(r)}\b", "", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        ...
        private async Task<List<List<(double lat, double lon)>>> GetStreetPolylinesAsync(string streetInput, CancellationToken ct)
        {
            ...
            var radius = 10000; // <-- increase a bit for full city coverage
